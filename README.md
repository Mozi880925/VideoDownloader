...
个人使用的桌面应用，定位是**「辅助发现并进入蓝海赛道、提高成功概率的工具」**：从下载器起步，现已串成一条完整工作流——

**发现赛道**（蓝海雷达关键词扫描 → 新锐频道榜单）→ **盯对标**（一键加入订阅，双栏跟踪频道动态）→ **拆爆款**（播放量快照探测爆款 + AI 拆解标题/频道打法）→ **挖内容**（字幕提取 / Whisper 转录 + AI 提纯为分享式原文）→ **沉淀选题**（全链路一键存为选题，汇入选题灵感库排期）。

下载视频不再是主打功能，而是这条工作流里「挖内容」环节的基础能力之一。底层使用 yt-dlp + YouTube Data API + LLM API（OpenAI 兼容）+ 飞书开放平台。
...
- 提纯稿库：查看、导出 Markdown、**一键交付飞书文档**、一键存为选题
- 字幕文稿查看器（搜索/复制/导出）
