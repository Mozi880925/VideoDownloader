...
个人使用的「辅助发现并进入蓝海赛道、提高成功概率」的桌面工作台：从视频
下载器起步，现已串成「发现赛道（蓝海雷达）→ 盯对标（频道订阅）→ 拆爆款
（AI 拆解）→ 挖内容（字幕/转录/提纯）→ 沉淀选题（选题灵感库）」完整工作
流。下载不再是主打功能，只是「挖内容」环节的基础能力。具体包含：下载
（单视频/批量/搜索/播放列表）、频道订阅监控、播放量快照与爆款探测、
...
顺序对齐「发现赛道→盯对标→拆爆款→挖内容→沉淀选题」工作流，下载工具后置为分组：

- 蓝海雷达（关键词管理 + 扫描 + 新锐频道榜单，榜单可一键「加入对标」订阅）
- 频道订阅（双栏：频道列表 + 视频流，含爆款探测/AI 拆解/文案提取/存为选题）
- 字幕和转录（合并页，胶囊 Tab 切换）：AI 识别字幕（含 URL/播客转录）|
  字幕提取 | 提纯稿库（AI 提纯 + 飞书交付，可存为选题）；Whisper 引擎配置在
  「设置 → 字幕设置」，非独立导航项
- 选题灵感库
- （分割线）
- 下载工具（分组）：视频下载（单视频 | 搜索 | 播放列表）、批量下载、下载列表
- 网络（代理 + 连通性测试）
- 设置（常规/字幕/Cookie/AI 与数据源/系统）
- 关于

默认落地页为蓝海雷达（navStore 初始 currentPage），下载工具不再是首屏。
